...
for key, value in DFManu.items():
    DFManu[key] = [value]


if st.button('Prédire'):
    #input_features = [np.array(DFManu)]
    prediction = model.predict_proba(pd.DataFrame.from_dict(DFManu))[:, 1]  # Probabilité de défaut de paiement
    st.write(f'Probabilité de mauvais payeur : {prediction[0]:.2f}')